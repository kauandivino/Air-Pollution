...
def load_similarity_results() -> pd.DataFrame:
    checkpoint_dir = METRICS_DIR / "loco_similarity_weighting_checkpoints"
    paths = sorted(checkpoint_dir.glob("*.csv"))
    if not paths:
        raise FileNotFoundError("No similarity-weighting checkpoints found.")
    data = pd.concat((pd.read_csv(path) for path in paths), ignore_index=True)
...
def paired_comparison(local: pd.DataFrame, weighted: pd.DataFrame) -> pd.DataFrame:
    ...
    merged = local[columns].merge(
        weighted[columns + ["feature_regime", "temperature", "weighting_strategy"]],
...
def summarize(deltas: pd.DataFrame) -> pd.DataFrame:
    rows = []
    for (target, model, temperature), group in deltas.groupby(
...
def main() -> None:
    ensure_results_dirs()
    weighted = load_similarity_results()
    all_deltas = []

    for (target, model, temperature), group in weighted.groupby(
        ["target", "model", "temperature"],
        observed=True,
    ):
    ...
    if not all_deltas:
        raise RuntimeError("No similarity-weighting experiments available for comparison.")

    deltas = pd.concat(all_deltas, ignore_index=True)
    summary = summarize(deltas)
    tests = wilcoxon_tests(deltas)

    delta_path = TABLES_DIR / "similarity_weighting_vs_local_delta_by_country.csv"
    summary_path = TABLES_DIR / "similarity_weighting_vs_local_summary.csv"
    tests_path = TABLES_DIR / "similarity_weighting_vs_local_wilcoxon.csv"
    ...
    print("Similarity weighting vs local features:")
    print(
        headline[
            [
                "target",
    ...
    print(
        tests[tests["metric"].isin(["pr_auc", "f1", "recall", "precision"])][
            ["target", "model", "temperature", "metric", "mean_delta", "p_value"]
        ].to_string(index=False)
    )
    print()
